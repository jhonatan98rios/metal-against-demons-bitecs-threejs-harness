...
// Validation
const ValidatedRelation = createRelation(withValidation((target) => target > 0))
```
...
const serializer = createObserverSerializer(
  world,
  Networked,
  [Position, Health],
  {
...
const deserializer = createObserverDeserializer(
  world,
  Networked,
  [Position, Health],
  {
...
const world = createWorld({
  components: {
    Position: {
      x: new Float32Array(
        new SharedArrayBuffer(MAX_ENTS * Float32Array.BYTES_PER_ELEMENT)
      ),
      y: new Float32Array(
        new SharedArrayBuffer(MAX_ENTS * Float32Array.BYTES_PER_ELEMENT)
      )
...
const removeQueue = new Uint32Array(
  new SharedArrayBuffer(MAX_CMDS * Uint32Array.BYTES_PER_ELEMENT)
)
...
const Position = {
  x: new Float32Array(MAX_ENTS),
  y: new Float32Array(MAX_ENTS)
}
const Velocity = {
  x: new Float32Array(MAX_ENTS),
  y: new Float32Array(MAX_ENTS)
...
observe(world, onAdd(Position, Velocity), (eid) =>
  world.enteredMovers.push(eid)
)
observe(world, onRemove(Position, Velocity), (eid) =>
  world.exitedMovers.push(eid)
)
...
// Server
const observerSerializer = createObserverSerializer(
  world,
  Networked,
  components
)
